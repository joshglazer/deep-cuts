...
### Props: don't add one until a caller needs it
...
### Assert on mock calls with matchers, not raw `.mock.calls[n]` indexing
...
### Don't add `beforeEach(() => vi.clearAllMocks())` — it's global
...
### `@/lib/amplify-server` is already mocked globally — don't re-mock it
...
### `@/auth` doesn't resolve under Vitest — mock it per-file, not globally
...
### Don't mock a child component without a specific, checkable reason
