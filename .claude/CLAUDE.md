...
generated by the Amplify sandbox and is git-ignored — it doesn't exist yet in
a freshly created worktree. Any page that touches `dataClient` (e.g.
`/queue`, `/queue/artist/[artistId]`) fails to compile with `Module not
found: Can't resolve '../../amplify_outputs.json'` until it's generated.
Before starting the dev server to preview a worktree, run `npm run sandbox`
(`ampx sandbox --profile deep-cuts`) in that worktree and leave it running in
the background — it watches `amplify/**` and keeps `amplify_outputs.json` in
sync.
