export interface ListenEventLike {
  playedAt: string;
}
...
// Date keys and all arithmetic below use local calendar days (getDate/getMonth/
// getDay, not the UTC variants) rather than UTC, since this module only ever
// runs client-side (see StatsView) so "local" reflects the visitor's actual
...
function buildHeatmap(dailyCounts: Map<string, number>, todayKey: string): HeatmapDay[] {
  const todayDayOfWeek = parseDateKey(todayKey).getDay();
...
export function computeStats(events: ListenEventLike[], now: Date): StatsData {
  const dailyCounts = new Map<string, number>();
  for (const event of events) {
    const dateKey = toDateKey(new Date(event.playedAt));
  ...
  const dayOfWeek = parseDateKey(todayKey).getDay();
  ...
  const previousMonthStartKey = toDateKey(
    new Date(previousMonthEndDate.getFullYear(), previousMonthEndDate.getMonth(), 1)
  );
